...
public class SaveLoadC : MonoBehaviour {
	...
	void OnGUI(){
		if(menu){
			GUI.Box(new Rect(Screen.width / 2 - 110,230,220,200), "存档");
			if(GUI.Button(new Rect(Screen.width / 2 - 45,285,90,40), "保存进度")){
				SaveData();
			}
			if(GUI.Button(new Rect(Screen.width / 2 - 45,365,90,40), "读取上次进度")){
				LoadData();
			}
